...
function SvgCreate(expand)
{
	...
	nbGenAsc = FsearchAsc + 1;
	nbGenDsc = FsearchDsc + 1;
	nbGenAscFound = 0;
	nbGenDscFound = 0;
	$(window).load(SvgInit);
	var html = "<div id='svgDrawing' class='" + (expand ? "svgDrawingExpand" : "svgDrawing") + "'>";
	// Graph type selector floating div
	html += "<div id='svgDrawingType' class='svgDrawingType'>";
	html += "<select name='svgTypeSelector' id='svgTypeSelector' class='svgTypeSelector' size='1'>";
	for (i = 0; i < patronsDoIt.length; i++)
	{
		html += "<option value='" + i + "'" + ((FsearchSvgType == i) ? " selected" : "") + ">" + SVG_TREE_TYPES_NAMES[i] + "</option>";
	}
	html += "</select></div>";
	// Floating popup div
	html += "<div id='svgPopup' class='svgPopup'>";
	html += "</div>";
	html += "</div>";
	return(html);
}
...
function SvgInit()
{
	// Iniitialize graph dimensions
	patronsDoIt0[FsearchSvgType]();
	CalcBoundingBox();
	svgPaper = new Raphael("svgDrawing", DimX, DimY);
	svgPaper.setViewBox(0, 0, viewBoxW, viewBoxH, true);
	svgRoot = svgPaper.canvas;
	// Graph type selector event
	$("#svgTypeSelector").change(function()
	{
		var v = $(this).val();
		FsearchSvgType = v;
		treeRef();
	...
	// Floating popup
	SvgPopupHide();
	$("#svgPopup").mousemove(SvgPopupMove);
	...
	var gbut1 = document.createElementNS('http://www.w3.org/2000/svg', 'g');
	gbut1.id = "buttons_group_1";
	svgRoot.appendChild(gbut1);
	svgPaper.canvas = gbut1;
	gbut1.setAttribute("transform",  "matrix(1,0,0,1,0,0)");
	var s = 16;
	var m = 4;
	var but = svgPaper.rect(viewBoxW - s - m, viewBoxH - s - m, s, s);
	but.node.setAttribute("class", "SVGbutton");
	but.id = "SvgZoomOut";
	var buttext = svgPaper.path(["M", viewBoxW - 2*m, viewBoxH - m-s/2, "l", -s+2*m, 0]);
	buttext.node.setAttribute("class", "SVGbuttontext");
	buttext.id = "SvgZoomOutText";
	but.click(SvgZoomOut);
	buttext.click(SvgZoomOut);
	var but = svgPaper.rect(viewBoxW - s - m, viewBoxH - 2*s - 2*m, s, s);
	but.node.setAttribute("class", "SVGbutton");
	but.id = "SvgZoomIn";
	var buttext = svgPaper.path(["M", viewBoxW -2*m, viewBoxH -2*m-s*1.5, "l", -s+2*m, 0, "m", s/2-m, -s/2+m, "l", 0, s-2*m]);
	buttext.node.setAttribute("class", "SVGbuttontext");
	buttext.id = "SvgZoomInText";
	but.click(SvgZoomIn);
	buttext.click(SvgZoomIn);
	// Create the expand button
	if ($(".svgDrawingExpand").length == 0)
	{
		var gbut2 = document.createElementNS('http://www.w3.org/2000/svg', 'g');
		gbut2.id = "buttons_group_2";
		svgRoot.appendChild(gbut2);
		svgPaper.canvas = gbut2;
		gbut2.setAttribute("transform",  "matrix(1,0,0,1,0,0)");
		var but = svgPaper.rect(viewBoxW - s - m, m, s, s);
		but.node.setAttribute("class", "SVGbutton");
		but.id = "SvgExpand";
		var arrow = svgPaper.path(["M", viewBoxW - s, s, "l", s-2*m, -s+2*m, "m", 0, 0, "l", m-s/2, 0, "m", -m+s/2, 0, "l", 0, -m+s/2]);
		arrow.node.setAttribute("class", "SVGbuttontext");
		arrow.id = "SVGexpandarrow";
		but.click(SvgExpand);
		arrow.click(SvgExpand);
	}
	else
	{
		...
		but.node.setAttribute("class", "SVGbutton");
		but.id = "SvgMinimize";
		var arrow = svgPaper.path(["M", viewBoxW -2*m, 2*m, "l", -s+2*m, s-2*m, "m", 0, 0, "l", -m+s/2, 0, "m", m-s/2, 0, "l", 0, m-s/2]);
		arrow.node.setAttribute("class", "SVGbuttontext");
		arrow.id = "SVGminimizearrow";
		but.click(SvgMinimize);
		arrow.click(SvgMinimize);
	}
	svgPaper.canvas = g;
...
function CalcBoundingBox()
{
	if ($(".svgDrawingExpand").length == 0)
	{
		var w = $("#svgDrawing").innerWidth();
		var h = $("#svgDrawing").innerHeight();
		var dim = innerDivNetSize(bodyContentsMaxSize(), $("#svgDrawing"));
		DimX = Math.max(77, Math.min(w, dim.width));
	...
	// console.log($("#svgDrawing").width(), $("#svgDrawing").height(), DimX, DimY);
...
function SvgSetStyle(p, text, x_elt, lev)
{
	...
	if (typeof(lev) != 'undefined') cl += " gen" + (lev + 1);
...
function SvgMouseEventExit()
{
	if (hoverBox >= 0)
	{
		...
		p.node.setAttribute("class", cl.replace(" SVGhover", ""));
...
function SvgMouseEventEnter(elt)
{
	if (elt >= 0 && elt != hoverBox)
	{
		...
		if (p)
		{
			hoverBox = elt;
			var cl = p.node.getAttribute("class");
			p.node.setAttribute("class", cl + " SVGhover");
...
function SvgResize()
{
	...
	if ($(".svgDrawingExpand").length == 0) return;
	// Compute new size
	var _DimX = DimX;
	var _DimY = DimY;
...
function SvgPopupMove(event)
{
	...
	p.x += m;
	p.y += m;
	if (p.x > DimX / 2) p.x -= $("#svgPopup").outerWidth() + 2 * m;
	if (p.y > DimY / 2) p.y -= $("#svgPopup").outerHeight() + 2 * m;
	$("#svgPopup").css("left", p.x);
	$("#svgPopup").css("top", p.y);
	return(true);
}
...
function doItAsc()
{
	...
	doItAscSub0(0, 0, 0, 2*Math.PI, depNum);
}

function doItAscSub0(x_elt, lev, a, b, num)
{
...
function doItAscHemi()
{
	...
	doItAscSub0(0, 0, -Math.PI/2 - hemiA, Math.PI/2 + hemiA, depNum);
}


//=========================================== Roue ascendante proportionnelle
function doIt0AscProp()
{
	doIt0Asc();
}

function doItAscProp()
{
	calcAsc(searchIdx, 0);
	calcRayonsPropAsc(searchIdx);
	svgElts = svgParents;
	maxTextWidth = coordX * rayons[0] * 2;
	svgElts[0][SVGELT_CMD].push("cCircle(" + [rayons[0], 0].join(", ") + ");");
	doItAscPropSub0(0, 0, 0, 2*Math.PI, depNum);
}

function doItAscPropSub0(x_elt, lev, a, b, num)
{
	var c = a;
	for (var i = 0; i < svgElts[x_elt][SVGELT_NEXT].length; i++)
	{
		...
		doItAscPropSub(x_next, lev + 1, c, c+da, (sex == "F") ? num * 2 + 1 : num * 2);
		c += da;
	}
}

function doItAscPropSub(x_elt, lev, a, b, num)
{
	svgElts[x_elt][SVGELT_CMD].push("secteur(" + [a, b, rayons[lev-1], rayons[lev], x_elt, lev].join(",") + ");");
	doItAscPropSub0(x_elt, lev, a, b, num);
}

//=========================================== Hemisphere ascendant proportionnel
function doIt0AscHemiProp()
{
	doIt0AscHemi();
}

function doItAscHemiProp()
{
	calcAsc(searchIdx, 0);
	calcRayonsPropAsc(searchIdx);
	svgElts = svgParents;
	maxTextWidth = coordX * rayons[0] * 2;
	svgParents[0][SVGELT_CMD].push("cCircleHemi(" + [rayons[0], -Math.PI/2 - hemiA, Math.PI/2 + hemiA, 0].join(", ") + ");");
	doItAscPropSub0(0, 0, -Math.PI/2 - hemiA, Math.PI/2 + hemiA, depNum);
}
...
function doItDsc()
{
	calcDsc(searchIdx, 0, false);
	calcRayonsPropDsc(searchIdx, false);
	svgElts = svgChildren;
	maxTextWidth = coordX * rayons[0] * 2;
	svgElts[0][SVGELT_CMD].push("cCircle(" + [rayons[0], 0].join(", ") + ");");
	doItDscSub0(0, 0, 0, 2*Math.PI);
}

function doItDscSub0(x_elt, lev, a, b)
{
	var c = a;
	for (var i = 0; i < svgElts[x_elt][SVGELT_NEXT].length; i++)
	{
		var x_next = svgElts[x_elt][SVGELT_NEXT][i];
		var da = 1.0 * (b - a) * svgElts[x_next][SVGELT_NB] / svgElts[x_elt][SVGELT_NB];
		doItDscSub(x_next, lev + 1, c, c + da);
		c += da;
	}
}

// function doItDscSub1(x_elt, lev, a, b)
// {
	// var i;
	...
	// {
		...
		// doItDscSub2(getChil(fdx)[i], lev, c, c+da);
		// c += da;
	// }
// }

function doItDscSub(x_elt, lev, a, b)
{
	svgElts[x_elt][SVGELT_CMD].push("secteur(" + [a, b, rayons[lev-1], rayons[lev], x_elt, lev].join(",") + ");");
	doItDscSub0(x_elt, lev, a, b);
}

//=========================================== Hemisphere descendant
function doIt0DscHemi()
{
	doIt0AscHemi();
}

function doItDscHemi()
{
	calcDsc(searchIdx, 0, false);
	calcRayonsPropDsc(searchIdx, false);
	svgElts = svgChildren;
	...
	doItDscSub0(0, 0, -Math.PI/2 - hemiA, Math.PI/2 + hemiA);
}


//=========================================== Roue descendante avec epoux
function doIt0DscSpou()
{
	doIt0Asc();
}

function doItDscSpou()
{
	calcDsc(searchIdx, 0, true);
	calcRayonsPropDsc(searchIdx, true);
	svgElts = svgChildren;
	maxTextWidth = coordX * rayons[0] * 2;
	svgElts[0][SVGELT_CMD].push("cCircle(" + [rayons[0], 0].join(", ") + ");");
	doItDscSpouSub0(0, 0, 0, 2*Math.PI);
}

function doItDscSpouSub0(x_elt, lev, a, b)
{
	var c = a;
	for (var i = 0; i < svgElts[x_elt][SVGELT_NEXT_SPOU].length; i++)
	{
		...
		var da = 1.0 * (b - a) * svgElts[x_next][SVGELT_NB] / svgElts[x_elt][SVGELT_NB];
		doItDscSpouSub1(x_next, lev, c, c + da);
		c += da;
	}
	c = a;
	for (var i = 0; i < svgElts[x_elt][SVGELT_NEXT].length; i++)
...
function doItDscHemiSpou()
{
	...
	maxTextWidth = coordX * rayons[0] * 2;
	svgElts[0][SVGELT_CMD].push("cCircleHemi(" + [rayons[0], -Math.PI/2 - hemiA, Math.PI/2 + hemiA, 0].join(", ") + ");");
	doItDscSpouSub0(0, 0, -Math.PI/2 - hemiA, Math.PI/2 + hemiA);
}


//=========================================== Roue ascendante+descendante

function doItBothSub(prop, print_spouses)
{
	calcAsc(searchIdx, 0);
	calcDsc(searchIdx, 0, print_spouses);
	calcRayonsPropBoth(searchIdx, print_spouses);
	mergeSVGelts();
	...
	rayons = rayonsAsc;
	if (prop) doItAscPropSub0(0, 0, -Math.PI/2, Math.PI/2, depNum);
	else doItAscSub0(0, 0, -Math.PI/2, Math.PI/2, depNum);
	// Descendants
	svgElts[0] = center2;
	maxTextWidth = coordX * rayonsDsc[0] * 2;
	rayons = rayonsDsc;
	if (print_spouses) doItDscSpouSub0(0, 0, Math.PI/2, 3*Math.PI/2);
	else doItDscSub0(0, 0, Math.PI/2, 3*Math.PI/2);
	// Texte central
...
function doItBoth()
{
	doItBothSub(false, false);
}

function doIt0BothProp()
{
	doIt0Asc();
}
function doItBothProp()
{
	doItBothSub(true, false);
}

function doIt0BothSpou()
{
	doIt0Asc();
}
function doItBothSpou()
{
	doItBothSub(false, true);
}

function doIt0BothPropSpou()
{
	doIt0Asc();
}
function doItBothPropSpou()
{
	doItBothSub(true, true);
}


//=========================================== Arbre ascendant
function doIt0AscTree()
{
	calcAsc(searchIdx, 0);
	svgElts = svgParents;
...
function doItAscTree()
{
	doItAscTreeSub(0, 0, svgElts[0][SVGELT_NB], depNum, nbGenAscFound, 0);
}

function doItAscTreeSub(x_elt, a, b, num, nb_gens, offsetx, child_x, child_y)
{
	...
	for (var i = 0; i < svgElts[x_elt][SVGELT_NEXT].length; i++)
	{
		...
		var sex = I[svgElts[x_next][SVGELT_IDX]][I_GENDER];
		var da = 1.0 * (b - a) * svgElts[x_next][SVGELT_NB] / svgElts[x_elt][SVGELT_NB];
		doItAscTreeSub(x_next, c, c + da, (sex == "F") ? num * 2 + 1 : num * 2, nb_gens, offsetx, x + box_width, y);
		c += da;
	}
}

//=========================================== Arbre descendant
function doIt0DscTree()
{
	calcDsc(searchIdx, 0, false);
	svgElts = svgChildren;
...
function doItDscTree()
{
	doItDscTreeSub(0, 0, svgElts[0][SVGELT_NB], nbGenDscFound, nbGenDscFound - 1, true);
}

function doItDscTreeSub(x_elt, a, b, nb_gens, offsetx, print_center, parent_x, parent_y)
{
	var lev = svgElts[x_elt][SVGELT_LEV];
	var box_width = Math.min(coordX / nb_gens, coordY / svgElts[0][SVGELT_NB] * txtRatio);
	var box_height = Math.min(box_width, svgElts[x_elt][SVGELT_NB] * box_width / txtRatio);
	...
	for (var i = 0; i < svgElts[x_elt][SVGELT_NEXT].length; i++)
	{
		var x_next = svgElts[x_elt][SVGELT_NEXT][i];
		var da = 1.0 * (b - a) * svgElts[x_next][SVGELT_NB] / svgElts[x_elt][SVGELT_NB];
		doItDscTreeSub(x_next, c, c + da, nb_gens, offsetx, print_center, x, y);
		c += da;
	}
}

//=========================================== Arbre descendant avec epoux
function doIt0DscTreeSpou()
{
	calcDsc(searchIdx, 0, true);
	svgElts = svgChildren;
...
function doItDscTreeSpou()
{
	var nb_gens = (nbGenDscFound * 2 - 1);
	doItDscTreeSpouSub(0, 0, svgElts[0][SVGELT_NB], nbGenDscFound * 2 - 1, nb_gens - 1, true);
}

function doItDscTreeSpouSub(x_elt, a, b, nb_gens, offsetx, print_center, parent_x, parent_y)
{
	var lev = svgElts[x_elt][SVGELT_LEV];
	var box_width = Math.min(coordX / nb_gens, coordY / svgElts[0][SVGELT_NB] * txtRatio);
	var box_height = Math.min(box_width, svgElts[x_elt][SVGELT_NB] * box_width / txtRatio);
	...
	for (var i = 0; i < svgElts[x_elt][SVGELT_NEXT].length; i++)
	{
		var x_next = svgElts[x_elt][SVGELT_NEXT][i];
		var da = 1.0 * (b - a) * svgElts[x_next][SVGELT_NB] / svgElts[x_elt][SVGELT_NB];
		doItDscTreeSpouSub(x_next, c, c + da, nb_gens, offsetx, print_center, x + spou_offset, y);
		c += da;
	}
	c = a;
	for (var i = 0; i < svgElts[x_elt][SVGELT_NEXT_SPOU].length; i++)
...
function doIt0BothTreeSub(prop, print_spouses)
{
	calcAsc(searchIdx, 0);
	calcDsc(searchIdx, 0, print_spouses);
...
function doItBothTreeSub(prop, print_spouses)
{
	var center1 = svgParents[0];
	var center2 = svgChildren[0];
	var nb_gens = nbGenAscFound + (print_spouses ? (nbGenDscFound * 2 - 1) : nbGenDscFound) - 1;
	// Ascendants
	svgElts[0] = center1;
	if (prop) doItAscTreeSub(0, 0, svgElts[0][SVGELT_NB], depNum, nb_gens, nb_gens - nbGenAscFound);
	else ;
	// Descendants
	svgElts[0] = center2;
	if (print_spouses) doItDscTreeSpouSub(0, 0, svgElts[0][SVGELT_NB], nb_gens, nb_gens - nbGenAscFound, false);
	else doItDscTreeSub(0, 0, svgElts[0][SVGELT_NB], nb_gens, nb_gens - nbGenAscFound, false);
	// Texte central
	svgElts[0] = center1;
}

function doIt0BothTreeProp()
{
	doIt0BothTreeSub(true, false);
}
function doItBothTreeProp()
{
	doItBothTreeSub(true, false);
}

function doIt0BothTreePropSpou()
{
	doIt0BothTreeSub(true, true);
}
function doItBothTreePropSpou()
{
	doItBothTreeSub(true, true);
}
...
function line(x1, y1, x2, y2)
{
	...
	p.node.setAttribute("class", "SVGline");
}
